...
void IfExpr::pretty_print_at(std::ostream &out, precedence_t prec, int space) {
  ...
  if (prec == prec_none) {
    precedence_t letPrec = prec_let;
    precedence_t nonePrec = prec_none;
       
    out << "_if ";
    _if->pretty_print_at(out, nonePrec, 0);
    out << "\n";
    for (int i = 0; i < pos; i++) {
      out.put(' ');
    }
    out << "_then ";
    _then->pretty_print_at(out, letPrec, 0);
    out << "\n";
    for (int i = 0; i < pos; i++) {
      out.put(' ');
    }
    out << "_else ";
    _else->pretty_print_at(out, letPrec, 0);
         
  }
  else if(prec == prec_let) {
    precedence_t letPrec = prec_let;
    precedence_t nonePrec = prec_none;
    out << "(";
    out<<"_if ";
...
void EqualExpr::pretty_print_at(std::ostream &out, precedence_t prec, int space) {
  if (prec ==prec_none|prec == prec_let) {
    precedence_t lhsPrec=prec_add;
    precedence_t rhsPrec=prec_none;
    this->lhs->pretty_print_at(out,lhsPrec, 0);
    out << " == ";
    this->rhs->pretty_print_at(out,rhsPrec, 0);
  } else {
    precedence_t lhsPrec= prec_add;
    precedence_t rhsPrec= prec_none;
    out << "(";
    lhs->pretty_print_at(out, lhsPrec, 0);
    out << " == ";
    rhs->pretty_print_at(out, rhsPrec, 0);
    out << ")";
  }
}
